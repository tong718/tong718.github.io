...
function name_judge(id){
    var name_judge = /^[a-zA-Z0-9_-]{1,16}$/;//只可以字母和数字
    username=document.getElementById(id).value;

    if(username != ""){
        if(!name_judge.test(username)){
            document.getElementById(id).blur();
            alert("Number and characters only");
        }
    }else{
...
function phone_judge(id){
    var phone_judge = /\d{5}/;//最少五位电话号码
    phone=document.getElementById(id).value;
    if(phone != ""){
        if(!phone_judge.test(phone)){
...
function address_judge(id){
    var address_judge = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
    address=document.getElementById(id).value;
    if(address != ""){
        if(!address_judge.test(address)){
